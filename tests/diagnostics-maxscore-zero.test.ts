...
function buildCleanResponses(): CleanResponse[] {
  ...
  PARTICIPANTS.forEach((p, i) => {
    ...
    recs.push(
      cleanResp({ qmQuestionId: "q0", qmParticipantId, participantPseudonym, maxScore: 0, answerGiven: null, answerScore: 0, responseTime: null }),
    );
    recs.push(
...
function makeDb(): MockDb {
  ...
  PARTICIPANTS.forEach((_, i) => {
    const pid = `u${i + 1}`;
    responses.push({ id: `r${i}-0`, cycle_id: CYCLE, participant_id: pid, item_id: "q0", answer_given: null, answer_score: 0, response_time: null, result_status: null, created_at: `2026-05-01T00:01:0${i}Z` });
    responses.push({ id: `r${i}-1`, cycle_id: CYCLE, participant_id: pid, item_id: "q1", answer_given: "A", answer_score: Q1_SCORES[i], response_time: Q1_TIMES[i], result_status: null, created_at: `2026-05-01T00:02:0${i}Z` });
    responses.push({ id: `r${i}-2`, cycle_id: CYCLE, participant_id: pid, item_id: "q2", answer_given: "A", answer_score: Q2_SCORES[i], response_time: Q2_TIMES[i], result_status: null, created_at: `2026-05-01T00:03:0${i}Z` });
  });
  return {
